...
## Готовые артефакты (без необходимости переобучения)

Датасет и веса модели не включены в репозиторий (слишком тяжёлые для git).
Готовая обученная модель (60 эпох, метрики в `outputs/evaluation_report.json`):

- **Веса модели (best.pt):** https://drive.google.com/file/d/1_GmDDjec-qgFZLyPWpPfRojcOOCybj6U/view?usp=drive_link
- **Полный технический отчёт (PDF/DOCX):** см. `logo_detection_report.docx` в корне репозитория

Скачайте `best.pt` и положите в `weights/logo_detector/weights/best.pt`, чтобы сразу
запускать `evaluate.py` или `inference_service.py` без повторного обучения (Шаг 2 ниже).
